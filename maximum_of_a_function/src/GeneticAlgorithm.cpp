...
GeneticAlgorithm::GeneticAlgorithm(const std::string &settings_filename, const std::string &output_filename,
    ...
    // set the domain of definition for the fitness function
    fin >> this->domain_of_definition.first >> this->domain_of_definition.second;

    // set the params for the function
    fin >> std::get<0>(this->function_params) >> std::get<1>(this->function_params)
        >> std::get<2>(this->function_params);
...
void GeneticAlgorithm::generatePopulation() {
    ...
    for (size_t i = 0; i < this->population_size; ++i) {
        this->population.emplace_back(Chromosome(this->domain_of_definition, this->precision));
    }
}
...
long double GeneticAlgorithm::fitnessFunction(const Chromosome &chromosome) const {
    // compute the value of the chromosome only once since it's expensive to compute
    long double chromosome_value = chromosome.getValue();
    if (chromosome_value >= this->domain_of_definition.second) {
        return 0;
    }
    auto &[a, b, c] = this->function_params;
    return a * pow(chromosome_value, 2) + b * chromosome_value + c;
}
...
void GeneticAlgorithm::printInitialPopulation() const {
    ...
    for (size_t i = 0; i < this->population_size; ++i) {
        std::cout << std::setw(ceil(log10(this->population_size))) << i + 1 << ": "
                  << this->population[i].getStringRepresentationOfGenome() << " x= "
                  << std::setw(this->precision + 3) << std::fixed << std::setprecision(this->precision)
                  << this->population[i].getValue() << std::setprecision(ss) << " f="
...
void GeneticAlgorithm::printSelectedPopulationInfo() const {
    ...
    for (size_t i = 0; i < this->population_size; ++i) {
        std::cout << std::setw(ceil(log10(this->population_size))) << i << ": "
                  << this->selected_population[i].getStringRepresentationOfGenome() << " x= "
                  << std::setprecision(precision) << std::setw(this->precision + 3)
                  << this->selected_population[i].getValue() << std::setprecision(ss) << " f="
...
void GeneticAlgorithm::printPopulationInfoAfterCrossing() const {
    ...
    for (size_t i = 0; i < this->population_size; ++i) {
        std::cout << std::setw(ceil(log10(this->population_size))) << i + 1 << ": "
                  << this->selected_population[i].getStringRepresentationOfGenome()
                  << " x= " << std::setw(this->precision + 3) << std::setprecision(this->precision)
                  << this->selected_population[i].getValue() << std::setprecision(ss) << " f="
                  << this->fitnessFunction(this->selected_population[i]) << '\n';
    }
}
...
void GeneticAlgorithm::printPopulationInfoAfterMutation() const {
    ...
    for (size_t i = 0; i < this->population_size; ++i) {
        std::cout << std::setw(ceil(log10(this->population_size))) << i + 1 << ": "
                  << this->selected_population[i].getStringRepresentationOfGenome() << " x= "
                  << std::setw(this->precision + 3) << std::setprecision(this->precision)
                  << this->selected_population[i].getValue() << std::setprecision(ss) << " f="
                  << this->fitnessFunction(this->selected_population[i]) << '\n';
    }
}
